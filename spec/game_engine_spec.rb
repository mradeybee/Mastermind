...
describe Mastermind::GameEngine do
  let(:game) { Mastermind::GameEngine.new }
  ...
  describe '#partial_match' do
    it 'returns partial matches' do
      expect(game.partial_match(%w[g y r b], %w[r g b y])).to eql(4)
    end
  end

  describe '#game' do
    it 'plays the game' do
      allow(game.computer_code).to receive(:computer_choice).with(0).and_return(%w[r r r r])
  ...
  describe '#analysis' do
    it 'analyses the game' do
      ...
      expect(game.analysis(%w[r g b y], 2, 0)).to eql('done')
    end
  end
