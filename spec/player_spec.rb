 ...
 before :each do
  ...
  it 'detects invalid inputs' do
   ...
   it 'detects invalid inputs and returns to ask for input' do
    ...
    expect(player.invalid(0,["r","r","r","r"])).to eql("Play again")
  end

  describe "#hint" do
    it 'gets hint' do
      allow(player).to receive(:player_entry).and_return("hint")
      expect(player.hint(0,["r","r","r","r"])).to eql("hint")
    end
  end
end 