...
module Mastermind
  class GameEngine
    ...
    def winner
      ...
      puts @msg.win_msg(@final_time, @counter, @computer_code.join('')).to_s if @computer_code
      name_and_reset
    end
    ...
    def try_again(_exact, _partial)
      @final_time = (Time.now - @start_time).to_i
      puts @msg.game_over_msg(@computer_code.join(''), @final_time).to_s if @computer_code
      @counter = 0
      ask
    end
